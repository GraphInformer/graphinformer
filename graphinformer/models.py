...
class GraphSelfAttentionOld(torch.nn.Module):
    ...
    def forward(self, H, D, attention_mask):
        ...
        if self.Wroute is not None:
            ...
            context += torch.einsum("bhij,bijv->bhiv", attention_probs, WD)
...
class GraphSelfAttention(torch.nn.Module):
    ...
    def forward(self, H, route_data, attention_mask):
        ...
        if self.Wroute is not None:
            ...
            context += torch.einsum("bhij,bhijv->bhiv", attention_probs, Vr)
...
class GraphInformer(torch.nn.Module):
    ...
    Returns:
        ...
        pooled       output for pooled embedding
    """
    def __init__(self, config):
        super().__init__()
        self.config     = config
    ...
    def forward(self, node_ids, node_features, route_data, attention_mask=None, dists=None):
        ...
        pooled        = self.pooler(last_layer)
        return last_layer, pooled
...
class GINodeRegression(torch.nn.Module):
    ...
    def forward(self, node_ids, node_features, route_data, attention_mask=None, dists=None):
        last_layer, pooled = self.gi(node_ids, node_features, route_data=route_data, attention_mask=attention_mask, dists=dists)
...
class GraphBlock(torch.nn.Module):
    def __init__(self, conf, input_size=None):
        super().__init__()
        self.self_attention = GraphSelfAttention(conf, input_size=input_size)
        self.net = torch.nn.Sequential(
            torch.nn.Linear(conf.hidden_size, conf.hidden_size),
            torch.nn.Dropout(conf.hidden_dropout),
            torch.nn.ReLU(),
        )
        self.normalize = conf.normalize
        if self.normalize:
            self.layer_norm = LayerNorm(conf.hidden_size, eps=1e-12)

    def forward(self, H, D, attention_mask):
        H2 = self.self_attention(H, D, attention_mask)
        if self.normalize:
            H2 = self.layer_norm(H2)
        return self.net(H2)

class GraphBlockNodeRegression(torch.nn.Module):
    def __init__(self, conf):
        super().__init__()
        self.register_buffer("head_radius", torch.Tensor(conf.head_radius))
        self.layers = torch.nn.ModuleList(
                [GraphBlock(conf, conf.input_size)] + 
            [GraphBlock(conf) for _ in range(conf.num_layers - 1)]
        )
    ...
    def forward(self, H, D, attention_mask, dists):
        """
        H               hidden vectors   [batch, node, input_size]
        D               route data       [batch, node, node, route_f]
        attention_mask  1.0 if node exists  [batch, node]
        dists           distances [batch, node, node]
        """
        logit_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        logit_attention_mask = (1.0 - logit_attention_mask) * -10000.0
        rad = self.head_radius.view(1,-1, 1, 1)
        d_mask = (rad + 1e-3 < dists.unsqueeze(1)).float() * -10000.0
        ## d_mask has shape: [batch, head, node, node]
        logit_attention_mask = logit_attention_mask + d_mask
...
class GraphBlockClassification(torch.nn.Module):
    def __init__(self, conf):
        super().__init__()
        self.register_buffer("head_radius", torch.Tensor(conf.head_radius))
        self.layers = torch.nn.ModuleList(
        ...
        self.out = torch.nn.Sequential(
            torch.nn.Linear(conf.hidden_size, conf.hidden_size),
            torch.nn.ReLU(),
            torch.nn.Dropout(conf.final_dropout),
            torch.nn.Linear(conf.hidden_size, conf.graph_num_labels),
        )
    ...
    def forward(self, H, D, attention_mask, dists):
        ...
        ## average pooling
        masked = H * attention_mask.unsqueeze(-1)
        pooled = masked.sum(-2) / attention_mask.sum(-1).unsqueeze(-1)

        return self.out(pooled)

class GraphBlockFPClassification(torch.nn.Module):
    def __init__(self, conf):
        super().__init__()
        self.register_buffer("head_radius", torch.Tensor(conf.head_radius))
        self.layers = torch.nn.ModuleList(
    ...
    def forward(self, H, D, attention_mask, dists):
        ...
        for layer, out_layer in zip(self.layers, self.out_layers):
            H      = layer(H, D, logit_attention_mask)
            out    = out_layer(H) * attention_mask
            pooled = out.sum(-2)
            fp     = fp + pooled
