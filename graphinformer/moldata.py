...
class MolDataset(Dataset):
    def __init__(self, mols=[], node_prop=None, weave_features=False, bond_types=False):
        """
    ...
    def add_mol(self, mol, add_atom_types=True):
        ...
        route = np.zeros((A.shape[0]+1, A.shape[0]+1, num_route_features), dtype=np.float32)
        route[1:,1:,0] = (dists == 0)
        route[1:,1:,1] = (dists == 1)
        route[1:,1:,2] = (dists == 2)
        route[1:,1:,3] = (dists == 3)
    ...
    def __getitem__(self, idx):
        item = {
            ...
            "route_features": self.route_features[idx],
...
def mol_collate(batch, add_adj=False):
    ...
    if batch[0]["node_labels"] is not None:
        res["node_labels"] = pad_sequence([s["node_labels"] for s in batch], batch_first=True)
